...
/**
 * `<connex-checkbox-group>` — wraps multiple `<connex-checkbox>` items with a
 * shared label, helper text, error, warning, and orientation.
 *
 * @element connex-checkbox-group
 *
 ...
 *
 * @slot - One or more `<connex-checkbox>` children.
 ...
 */
@customElement('connex-checkbox-group')
export class ConnexCheckboxGroup extends LitElement {
  static styles = css`
    :host {
      display: block;
    ...
    .warning { color: var(--connex-text-display-warning); }
    .error { color: var(--connex-text-display-error); }
  `;
  ...
  updated(changed: Map<string, unknown>) {
    ...
    if (changed.has('error') || changed.has('disabled')) {
      const items = this.querySelectorAll('connex-checkbox');
      items.forEach((c) => {
...
declare global {
  interface HTMLElementTagNameMap {
    'connex-checkbox-group': ConnexCheckboxGroup;
  }
}
