...
const ICONS: Record<ControlType, ReturnType<typeof svg>> = {
  ...
  tooltip: svg`<path fill-rule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12Zm8.706-1.442c1.146-.573 2.437.463 2.126 1.706l-.732 2.923.305-.158a.75.75 0 0 1 .67 1.34l-.32.165c-1.146.573-2.437-.463-2.126-1.706l.732-2.923-.305.158a.75.75 0 1 1-.67-1.34l.32-.165ZM12 8.25a.75.75 0 1 0 0 1.5.75.75 0 0 0 0-1.5Z" clip-rule="evenodd"/>`,
...
/**
 * `<connex-control>` — interactive control that triggers a single action with
 ...
 */
@customElement('connex-control')
export class ConnexControl extends LitElement {
  static styles = css`
    :host {
      display: inline-flex;
      --_control-size: 32px;
      --_control-icon-size: 20px;
      --_control-fg: var(--connex-icon-interactive-primary);
      --_control-fg-hover: var(--connex-color-cool-gray-900);
    }
    :host([size='condensed']) {
      --_control-size: 24px;
    ...
    :host([color='critical']) {
      --_control-fg: var(--connex-icon-interactive-delete);
      --_control-fg-hover: var(--connex-color-red-700);
    }
    ...
    .control {
      ...
      transition:
        ...
        color var(--connex-motion-duration-fast)
          var(--connex-motion-easing-standard);
      flex-shrink: 0;
    }
    .control:hover:not(:disabled) {
      background: var(--connex-color-alpha-20);
