...
/**
 * `<connex-skeleton>` — placeholder shape that animates while content loads.
 *
 ...
 *
 * @cssprop --connex-skeleton-base   - Background color of the resting fill.
 * @cssprop --connex-skeleton-shimmer - Highlight color used by the shimmer pass.
 */
@customElement('connex-skeleton')
export class ConnexSkeleton extends LitElement {
  static styles = css`
    :host {
      display: block;
      ...
      --_sk-base: var(--connex-skeleton-base, var(--connex-color-cool-gray-100));
      --_sk-shimmer: var(--connex-skeleton-shimmer, var(--connex-color-cool-gray-200));
      --_sk-radius: var(--connex-radius-4);
      --_sk-w: 100%;
      --_sk-h: 14px;
    }
    ...
    :host([shape='block'])  { --_sk-w: 100%;  --_sk-h: 96px; --_sk-radius: var(--connex-radius-8); }
    :host([shape='circle']) {
      --_sk-w: 40px;
      --_sk-h: 40px;
      --_sk-radius: var(--connex-radius-999);
...
declare global {
  interface HTMLElementTagNameMap {
    'connex-skeleton': ConnexSkeleton;
  }
}
