...
/**
 * `<connex-progress-gauge>` — circular progress indicator.
 *
 ...
 *
 * @fires connex-progress-change - Bubbles, composed; detail = `{ value, max, ratio }`.
 */
@customElement('connex-progress-gauge')
export class ConnexProgressGauge extends LitElement {
  static styles = css`
    :host {
      display: inline-flex;
      ...
      --_pg-size: 64px;
      --_pg-stroke: 6px;
      --_pg-pct: var(--connex-font-size-14);
    }
    :host([size='small'])  { --_pg-size: 40px; --_pg-stroke: 4px; --_pg-pct: var(--connex-font-size-10); }
    :host([size='medium']) { --_pg-size: 64px; --_pg-stroke: 6px; --_pg-pct: var(--connex-font-size-14); }
    :host([size='large'])  { --_pg-size: 96px; --_pg-stroke: 8px; --_pg-pct: var(--connex-font-size-20); }
    :host([size='x-large']){ --_pg-size: 128px;--_pg-stroke: 10px;--_pg-pct: var(--connex-font-size-24); }
    ...
    .base {
      fill: none;
      stroke: var(--connex-color-cool-gray-200);
      stroke-width: var(--_pg-stroke);
    }
    .fill {
      fill: none;
      stroke: var(--connex-text-interactive-primary);
      stroke-width: var(--_pg-stroke);
      stroke-linecap: round;
      transition: stroke-dashoffset var(--connex-motion-duration-base, 240ms)
  ...
  updated(changed: Map<string, unknown>) {
    if (changed.has('value') || changed.has('max')) {
      this.dispatchEvent(
        new CustomEvent('connex-progress-change', {
          bubbles: true,
          composed: true,
...
declare global {
  interface HTMLElementTagNameMap {
    'connex-progress-gauge': ConnexProgressGauge;
  }
}
