...
/**
 * `<connex-progress-bar>` — linear progress indicator.
 *
 ...
 *
 * @fires connex-progress-change - Bubbles, composed; detail = `{ value, max, ratio }`. Fired when value or max changes.
 */
@customElement('connex-progress-bar')
export class ConnexProgressBar extends LitElement {
  static styles = css`
    :host {
      display: block;
    ...
    .fill {
      ...
      border-radius: 999px;
      transition: width var(--connex-motion-duration-base, 240ms)
...
declare global {
  interface HTMLElementTagNameMap {
    'connex-progress-bar': ConnexProgressBar;
  }
}
