...
/**
 * `<connex-accordion-item>` — a single expandable row within an accordion.
 *
 * Anatomy: trigger button (icon + label) + collapsible content region.
 *
 * @element connex-accordion-item
 *
 ...
 *
 * @fires connex-accordion-toggle - Bubbling, composed event with `{ expanded }` detail.
 */
@customElement('connex-accordion-item')
export class ConnexAccordionItem extends LitElement {
  static styles = css`
    :host {
      display: block;
      border-bottom: var(--_cnx-accordion-divider-width, 0) solid
        var(--connex-border-secondary);
    }
    :host(:last-of-type) {
      border-bottom: none;
    }
    ...
    .icon {
      ...
      transition: transform var(--connex-motion-duration-base)
        var(--connex-motion-easing-standard);
    }
    :host([expanded]) .icon {
    ...
    .content {
      ...
      transition: grid-template-rows var(--connex-motion-duration-base)
        var(--connex-motion-easing-standard);
    }
  ...
  private _toggle() {
    ...
    this.dispatchEvent(
      new CustomEvent<{ expanded: boolean }>('connex-accordion-toggle', {
        bubbles: true,
        composed: true,
...
declare global {
  interface HTMLElementTagNameMap {
    'connex-accordion-item': ConnexAccordionItem;
  }
}
