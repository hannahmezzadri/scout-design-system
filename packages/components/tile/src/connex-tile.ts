import { LitElement, html, css, nothing } from 'lit';
import { customElement, property } from 'lit/decorators.js';
import '@connex/skeleton';
import '@connex/show-more';
import type { TileFooter, TileFunctionalState } from './types.js';

/**
 * `<connex-tile>` — rich container with multi-part header and
 * optional footer. Used on overview / dashboard pages for static information
 * or to house multiple interactive elements (links, buttons, tables).
 *
 * Header anatomy (all slots optional except the title prop):
 *   - eyebrow      — small uppercase line above the title
 *   - title        — `header` prop
 *   - tooltip      — slot for a `<connex-tooltip>` next to the title
 *   - badge        — slot for a `<connex-badge>` next to the title
 *   - subhead      — `subhead` prop
 *   - header-button — slot for a button (e.g., overflow menu) on the header right
 *
 * Body: default slot — any content.
 *
 * Footer (controlled by `footer` attr):
 *   - none            — no footer
 *   - button-tertiary — slot a tertiary action button
 *   - show-more       — built-in `<connex-show-more>` toggle
 *
 * @element connex-tile
 *
 * @attr header                                                  - Title text.
 * @attr subhead                                                  - Optional subhead under the title.
 * @attr {"default"|"loading"|"error"} state                       - Functional state.
 * @attr {"none"|"button-tertiary"|"show-more"} footer             - Footer affordance.
 * @attr expanded                                                  - When `footer="show-more"`, reflects the open state.
 *
 * @slot eyebrow        - Eyebrow line above the title (typically text in uppercase).
 * @slot tooltip        - Inline tooltip next to the title.
 * @slot badge          - Badge next to the title.
 * @slot header-button  - Header right-side action.
 * @slot                - Body content.
 * @slot footer         - Footer content when `footer="button-tertiary"`.
 *
 * @fires connex-tile-show-more-toggle - Bubbles when show-more footer is toggled. detail = `{ expanded }`.
 */
@customElement('connex-tile')
export class ConnexTile extends LitElement {
  static styles = css`
    :host {
      display: block;
      font-family: var(--connex-font-family-inter);
    }

    .tile {
      /* Per spec: fill.white background + 1px solid cool-gray.200 border.
         The fill.white semantic stays white across themes (including dark). */
      background: var(--connex-fill-always-white);
      border: var(--connex-border-width-1) var(--connex-stroke-solid)
        var(--connex-color-cool-gray-200);
      border-radius: var(--connex-radius-8);
      padding: var(--connex-space-16);
      display: flex;
      flex-direction: column;
      gap: var(--connex-space-12);
    }
    /* Dark theme — swap the white fill for a semantic dark surface so the
       tile reads on a dark page. :host-context pierces the shadow boundary
       to read the theme attribute set on <html>. */
    :host-context([data-theme='dark']) .tile {
      background: var(--connex-surface-primary);
      border-color: var(--connex-border-primary);
    }

    /* Header --------------------------------------------------------- */
    .eyebrow {
      font-size: var(--connex-font-size-10);
      font-weight: var(--connex-font-weight-semibold);

      color: var(--connex-text-display-secondary);
    }
    .title-row {
      display: flex;
      align-items: flex-start;
      gap: var(--connex-space-8);
    }
    .title {
      flex: 1;
      min-width: 0;
      display: inline-flex;
      align-items: center;
      gap: var(--connex-space-8);
      font-size: var(--connex-font-size-16);
      font-weight: var(--connex-font-weight-semibold);
      line-height: var(--connex-font-line-height-24);
      color: var(--connex-text-display-primary);
    }
    .title h3 {
      margin: 0;
      font: inherit;
    }
    .header-button {
      flex-shrink: 0;
    }
    .subhead {
      font-size: var(--connex-font-size-12);
      color: var(--connex-text-display-secondary);
    }

    /* Body ----------------------------------------------------------- */
    .body {
      font-size: var(--connex-font-size-14);
      line-height: var(--connex-font-line-height-21);
      color: var(--connex-text-display-primary);
    }
    /* When show-more is the footer and the tile is collapsed, clamp the body */
    :host([footer='show-more']:not([expanded])) .body {
      display: -webkit-box;
      -webkit-line-clamp: 4;
      -webkit-box-orient: vertical;
      overflow: hidden;
    }

    /* Footer --------------------------------------------------------- */
    .footer { display: flex; align-items: center; }

    /* Loading -------------------------------------------------------- */
    .loading {
      display: flex;
      flex-direction: column;
      gap: var(--connex-space-8);
    }
    /* Error ---------------------------------------------------------- */
    .error {
      display: inline-flex;
      align-items: center;
      gap: var(--connex-space-8);
      color: var(--connex-text-display-error);
      font-size: var(--connex-font-size-14);
    }
    .error svg { width: 20px; height: 20px; fill: currentColor; flex-shrink: 0; }
  `;

  @property() header = '';
  @property() subhead = '';
  @property({ reflect: true }) state: TileFunctionalState = 'default';
  @property({ reflect: true }) footer: TileFooter = 'none';
  @property({ type: Boolean, reflect: true }) expanded = false;

  private _onShowMoreToggle = (e: Event) => {
    const detail = (e as CustomEvent<{ expanded: boolean }>).detail;
    this.expanded = detail.expanded;
    this.dispatchEvent(
      new CustomEvent<{ expanded: boolean }>('connex-tile-show-more-toggle', {
        bubbles: true,
        composed: true,
        detail: { expanded: this.expanded },
      }),
    );
  };

  private _renderBody() {
    if (this.state === 'loading') {
      return html`<div class="loading">
        <connex-skeleton shape="line"></connex-skeleton>
        <connex-skeleton shape="line"></connex-skeleton>
        <connex-skeleton shape="line" width="70%"></connex-skeleton>
      </div>`;
    }
    if (this.state === 'error') {
      return html`<div class="error" role="alert">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path fill-rule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12Zm9.75-3.75a.75.75 0 0 1 .75.75v4.5a.75.75 0 0 1-1.5 0v-4.5a.75.75 0 0 1 .75-.75Zm0 8.25a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5Z" clip-rule="evenodd"/>
        </svg>
        <span><slot name="error-message">Couldn't load this tile.</slot></span>
      </div>`;
    }
    return html`<div class="body"><slot></slot></div>`;
  }

  private _renderFooter() {
    if (this.state !== 'default') return nothing;
    if (this.footer === 'none') return nothing;
    if (this.footer === 'show-more') {
      return html`<div class="footer">
        <connex-show-more
          ?expanded=${this.expanded}
          @connex-show-more-toggle=${this._onShowMoreToggle}
        ></connex-show-more>
      </div>`;
    }
    // button-tertiary — consumer slots their own button
    return html`<div class="footer"><slot name="footer"></slot></div>`;
  }

  render() {
    return html`
      <div class="tile">
        <div class="eyebrow"><slot name="eyebrow"></slot></div>
        <div class="title-row">
          <span class="title">
            <h3>${this.header}</h3>
            <slot name="tooltip"></slot>
            <slot name="badge"></slot>
          </span>
          <span class="header-button"><slot name="header-button"></slot></span>
        </div>
        ${this.subhead ? html`<div class="subhead">${this.subhead}</div>` : nothing}
        ${this._renderBody()}
        ${this._renderFooter()}
      </div>
    `;
  }
}

declare global {
  interface HTMLElementTagNameMap {
    'connex-tile': ConnexTile;
  }
}
