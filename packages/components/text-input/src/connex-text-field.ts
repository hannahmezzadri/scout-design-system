...
/**
 * `<connex-text-field>` — single-line text input with eleven variants.
 *
 ...
 * @fires connex-text-field-trigger - Bubbles, composed; fires when the trailing
 *                                     picker icon (date / month / time) is clicked.
 *                                     detail = `{ variant }`.
 */
@customElement('connex-text-field')
export class ConnexTextField extends LitElement {
  ...
  static styles = css`
    :host {
      ...
      --_tf-h: 36px;
      --_tf-fs: var(--connex-font-size-14);
      --_tf-px: var(--connex-space-12);
    }
    :host([size='condensed']) {
      --_tf-h: 28px;
      --_tf-fs: var(--connex-font-size-12);
      --_tf-px: var(--connex-space-8);
    }
    ...
    :host([data-active]:not(:focus-within)) .field {
      border-color: var(--connex-text-display-primary);
    }
    :host([invalid]) .field {
    ...
    .icon-btn:hover { color: var(--connex-text-display-primary); background: var(--connex-interactive-background-hover); }
    .icon-btn:focus-visible {
      outline: var(--connex-focus-ring-width) solid var(--connex-focus-ring-color);
      outline-offset: 1px;
    }

    .check-match { color: var(--connex-text-display-success); }
  ...
  private _firePickerTrigger = () => {
    this.dispatchEvent(
      new CustomEvent<{ variant: TextFieldVariant }>('connex-text-field-trigger', {
        bubbles: true,
        composed: true,
  ...
  private _confirmMatches(): boolean {
    ...
    const partner = root.querySelector<HTMLInputElement | ConnexTextField>(this.confirmTarget);
    if (!partner) return false;
    return (partner as ConnexTextField).value === this.value;
  }
...
declare global {
  interface HTMLElementTagNameMap {
    'connex-text-field': ConnexTextField;
  }
}
