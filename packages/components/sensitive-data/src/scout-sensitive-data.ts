...
/**
 * `<scout-sensitive-data>` — masks the slotted value by default with a
 ...
 *
 * @fires scout-sensitive-data-toggle - Bubbles, composed; detail = `{ revealed }`.
 */
@customElement('scout-sensitive-data')
export class ScoutSensitiveData extends LitElement {
  static styles = css`
    :host {
      display: inline-flex;
      align-items: center;
  ...
  private _toggle = () => {
    ...
    this.dispatchEvent(
      new CustomEvent<{ revealed: boolean }>('scout-sensitive-data-toggle', {
        bubbles: true,
        composed: true,
...
declare global {
  interface HTMLElementTagNameMap {
    'scout-sensitive-data': ScoutSensitiveData;
  }
}
