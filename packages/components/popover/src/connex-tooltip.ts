...
/**
 * `<connex-tooltip>` — popover tooltip anchored to a trigger.
 *
 ...
 *
 * @fires connex-tooltip-open  - Bubbles, composed; detail = `{ open: true }`.
 * @fires connex-tooltip-close - Bubbles, composed; detail = `{ open: false }`.
 */
@customElement('connex-tooltip')
export class ConnexTooltip extends LitElement {
  static styles = css`
    :host {
      display: inline-flex;
      position: relative;
  ...
  private _emit(name: 'open' | 'close') {
    this.dispatchEvent(
      new CustomEvent(`connex-tooltip-${name}`, {
        bubbles: true,
        composed: true,
