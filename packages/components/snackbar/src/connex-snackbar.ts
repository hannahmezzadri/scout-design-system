...
/**
 * `<connex-snackbar>` — temporary, low-impact toast notification.
 *
 ...
 *
 * @fires connex-snackbar-dismiss - Fires when the snackbar is removed (auto or user).
 */
@customElement('connex-snackbar')
export class ConnexSnackbar extends LitElement {
  static styles = css`
    :host {
    ...
    .snackbar {
      ...
      background: var(--connex-color-cool-gray-800);
    ...
    :host([status='success']) .status-icon { color: var(--connex-color-green-400); }
    :host([status='warning']) .status-icon { color: var(--connex-color-yellow-400); }
    :host([status='critical']) .status-icon { color: var(--connex-color-red-400); }
    ...
    /* X dismiss control — shown for success/warning, hidden for critical.
       The connex-control component is light-surface by default; we override
    ...
    .dismiss {
      flex-shrink: 0;
      --_control-fg: var(--connex-color-cool-gray-200);
      --_control-fg-hover: var(--connex-color-white);
    }
    .dismiss::part(button):hover {
      background: var(--connex-color-alpha-white-20);
    }
  `;
  ...
  private _dismiss = () => {
    this.dispatchEvent(
      new CustomEvent('connex-snackbar-dismiss', { bubbles: true, composed: true }),
    );
    this.remove();
  };
...
declare global {
  interface HTMLElementTagNameMap {
    'connex-snackbar': ConnexSnackbar;
  }
}
