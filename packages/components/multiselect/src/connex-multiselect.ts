...
import type { ConnexMultiselectOption } from './connex-multiselect-option.js';
...
/**
 * `<connex-multiselect>` — multi-select dropdown with searchable input,
 * chips for selected values, select-all, counter, and clear-all.
 *
 * @element connex-multiselect
 *
 * @attr label
 ...
 *
 * @slot - One or more `<connex-multiselect-option>` children.
 *
 * @fires connex-multiselect-change - Bubbling, composed; detail = `{ values: string[] }`.
 */
@customElement('connex-multiselect')
export class ConnexMultiselect extends LitElement {
  static styles = css`
    :host {
      display: block;
      width: 100%;
      ...
      --_ms-min-h: 36px;
      --_ms-fs: var(--connex-font-size-14);
    }
    :host([size='condensed']) {
      --_ms-min-h: 28px;
      --_ms-fs: var(--connex-font-size-12);
    }
    ...
    .input-wrap {
      ...
      padding: 4px var(--connex-space-12) 4px 4px;
    ...
    .chip-remove:hover { background: var(--connex-color-blue-200); }
  ...
  private get _options(): ConnexMultiselectOption[] {
    return Array.from(this.querySelectorAll('connex-multiselect-option'));
  }
  ...
  private _visibleOptions(): ConnexMultiselectOption[] {
  ...
  private _onMenuClick = (e: Event) => {
    const target = (e.target as HTMLElement).closest('connex-multiselect-option') as ConnexMultiselectOption | null;
  ...
  private _fireChange() {
    this.dispatchEvent(new CustomEvent<{ values: string[] }>('connex-multiselect-change', {
...
declare global {
  interface HTMLElementTagNameMap {
    'connex-multiselect': ConnexMultiselect;
  }
}
