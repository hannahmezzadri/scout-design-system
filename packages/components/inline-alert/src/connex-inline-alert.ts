...
/**
 * `<connex-inline-alert>` — contextual alert embedded in the page flow.
 *
 * @element connex-inline-alert
 *
 * @attr {"informational"|"favorable"|"warning"|"critical"} status - Visual + semantic status.
 * @attr {"default"|"condensed"} size - Padding density.
 * @attr closable - When set, renders a close button that fires `connex-inline-alert-close` and removes the host.
 *
 ...
 *
 * @fires connex-inline-alert-close - Bubbles, composed.
 */
@customElement('connex-inline-alert')
export class ConnexInlineAlert extends LitElement {
  static styles = css`
    :host {
      display: block;
      --_cnx-alert-padding-block: var(--connex-space-12);
      --_cnx-alert-padding-inline: var(--connex-space-16);
      --_cnx-alert-icon-size: 20px;
    }
    :host([size='condensed']) {
      --_cnx-alert-padding-block: var(--connex-space-8);
      --_cnx-alert-padding-inline: var(--connex-space-12);
    ...
    :host([status='informational']) .alert {
      ...
      border-color: var(--connex-border-info);
    }
    :host([status='informational']) .status-icon { color: var(--connex-text-display-info); }

    :host([status='favorable']) .alert {
      background: var(--connex-color-green-100);
      border-color: var(--connex-border-success);
    }
    :host([status='favorable']) .status-icon { color: var(--connex-text-display-success); }

    :host([status='warning']) .alert {
      background: var(--connex-color-yellow-100);
      border-color: var(--connex-border-warning);
    }
    :host([status='warning']) .status-icon { color: var(--connex-text-display-warning); }

    :host([status='critical']) .alert {
      background: var(--connex-color-red-100);
      border-color: var(--connex-border-error);
    }
    :host([status='critical']) .status-icon { color: var(--connex-text-display-error); }
  ...
  private _close = () => {
    this.dispatchEvent(
      new CustomEvent('connex-inline-alert-close', { bubbles: true, composed: true }),
    );
    this.remove();
  };
...
declare global {
  interface HTMLElementTagNameMap {
    'connex-inline-alert': ConnexInlineAlert;
  }
}
