...
/**
 * `<connex-pagination>` — page navigation for tables and paged content.
 *
 ...
 *
 * @fires connex-pagination-change - Bubbles, composed; detail = `{ page, pageSize }`.
 */
@customElement('connex-pagination')
export class ConnexPagination extends LitElement {
  static styles = css`
    :host {
      display: inline-flex;
      align-items: center;
      ...
      --_pg-fs: var(--connex-font-size-14);
      --_pg-gap: var(--connex-space-4);
    }
    :host([size='condensed']) {
      --_pg-control-size: 24px;
      --_pg-fs: var(--connex-font-size-12);
      --_pg-gap: var(--connex-space-2, 2px);
    }
    ...
    .ctrl[aria-current='page']:focus-visible {
      outline-color: var(--connex-focus-ring-color);
    }
  ...
  private _emit() {
    this.dispatchEvent(
      new CustomEvent<{ page: number; pageSize: number }>('connex-pagination-change', {
        bubbles: true,
        composed: true,
  ...
  private _renderItemsDropdown() {
    const dd = document.createElement('connex-dropdown-select');
    ...
    dd.addEventListener('connex-dropdown-change', this._onSizeChange as EventListener);
    return html`
      <div class="items">
...
declare global {
  interface HTMLElementTagNameMap {
    'connex-pagination': ConnexPagination;
  }
}
