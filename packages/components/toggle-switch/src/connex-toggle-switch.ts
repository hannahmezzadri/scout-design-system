...
/**
 * `<connex-toggle-switch>` — quickly switch between two states (On / Off).
 *
 ...
 */
@customElement('connex-toggle-switch')
export class ConnexToggleSwitch extends LitElement {
  ...
  static styles = css`
    :host {
      ...
      --_knob-size: 16px;
      --_knob-offset: 2px;
      --_track-off: var(--connex-color-cool-gray-300, var(--connex-color-cool-gray-200));
      --_track-on:  var(--connex-text-interactive-primary);
      --_track-on-pressed: var(--connex-color-blue-700);
      --_track-critical: var(--connex-color-red-600);
      --_track-critical-pressed: var(--connex-color-red-700);
    }
    :host([size='condensed']) {
      --_track-w: 28px;
      --_track-h: 16px;
    ...
    .track::after {
      ...
      background: var(--connex-color-white);
      border-radius: 50%;
      box-shadow: 0 1px 2px rgba(0, 0, 0, 0.18);
      transition: transform var(--connex-motion-duration-fast, 120ms)
        var(--connex-motion-easing-standard, ease);
    }

    /* Hover / pressed / focus on the track when not disabled */
    .row:hover .track { background: var(--connex-color-cool-gray-400, var(--connex-color-cool-gray-300)); }
    ...
    .row:active:not([aria-disabled='true']) .track {
      background: var(--connex-color-cool-gray-500, var(--connex-color-cool-gray-400));
    }

    /* Functional: ON */
    :host([checked]) .track { background: var(--_track-on); }
    :host([checked]) .row:hover .track { background: var(--connex-color-blue-600, var(--_track-on)); }
    ...
    :host([size='condensed']) .label { font-size: var(--connex-font-size-12); }
...
declare global {
  interface HTMLElementTagNameMap {
    'connex-toggle-switch': ConnexToggleSwitch;
  }
}
