...
export class ScoutToggleSwitch extends LitElement {
  ...
  static styles = css`
    ...
    /* Density — split into independent rules so Safari keeps the
       :host([size]) rule even if it drops :host-context(). */
    :host([size='condensed']) {
      --_track-w: 28px;
      --_track-h: 16px;
      --_knob-size: 12px;
    ...
    /* Label — default = body, condensed steps down to body-small to
       match the rest of the form-input family. Split rules for Safari. */
    .label {
