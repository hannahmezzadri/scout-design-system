...
/**
 * `<connex-breadcrumb>` — hierarchy navigation showing the user's location.
 *
 * Two configurations:
 * - **Multi (3+)**: a chain of `<connex-breadcrumb-item>` children, separated by chevrons.
 ...
 *
 * @element connex-breadcrumb
 *
 * @attr back - Switches to single back-link mode with a leading arrow icon.
 *
 * @slot - One or more `<connex-breadcrumb-item>` elements.
 */
@customElement('connex-breadcrumb')
export class ConnexBreadcrumb extends LitElement {
  static styles = css`
    :host {
      display: block;
    }
    ...
    .back-arrow {
      ...
      width: 16px;
      height: 16px;
      margin-right: var(--connex-space-4);
      color: var(--connex-text-interactive-primary);
    }
    :host([back]) ::slotted(connex-breadcrumb-item) {
  ...
  private _onSlotChange = (e: Event) => {
    ...
    const items = slot
      .assignedElements()
      .filter((n) => n.tagName === 'CONNEX-BREADCRUMB-ITEM') as HTMLElement[];
  ...
  updated() {
    ...
    const items = Array.from(
      this.querySelectorAll('connex-breadcrumb-item'),
    ) as HTMLElement[];
    items.forEach((item, i) => {
      item.toggleAttribute('data-last', i === items.length - 1);
      item.toggleAttribute('data-back', this.back);
...
declare global {
  interface HTMLElementTagNameMap {
    'connex-breadcrumb': ConnexBreadcrumb;
  }
}
