...
/**
 * `<connex-share-with-customer>` — agent-facing message displayed inside a
 ...
 *   - Label (top)
 *   - Optional `<connex-language-tabs>` for translations
 ...
 *
 * @element connex-share-with-customer
 *
 ...
 *
 * @fires connex-share-language-change - Bubbles, composed; detail = `{ value }`.
 */
@customElement('connex-share-with-customer')
export class ConnexShareWithCustomer extends LitElement {
  static styles = css`
    :host {
      display: block;
    ...
    .surface {
      ...
      background: var(--connex-fill-primary, var(--connex-color-blue-50));
      border: var(--connex-border-width-1) var(--connex-stroke-solid)
        var(--connex-color-blue-100);
  ...
  private _onLanguageChange = (e: Event) => {
    ...
    this.dispatchEvent(
      new CustomEvent<{ value: string }>('connex-share-language-change', {
        bubbles: true,
        composed: true,
        ...
        <div class="head">
          ...
          ${hasLangs
            ? html`<connex-language-tabs
                ...
                value=${this.value}
                @connex-language-change=${this._onLanguageChange}
              ></connex-language-tabs>`
            : nothing}
        </div>
        <div class="body">
...
declare global {
  interface HTMLElementTagNameMap {
    'connex-share-with-customer': ConnexShareWithCustomer;
  }
}
