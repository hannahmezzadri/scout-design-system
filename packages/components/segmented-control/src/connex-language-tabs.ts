...
/**
 * Connex language tabs are intentionally limited to the three languages
...
/**
 * `<connex-language-tabs>` — language picker built on top of
 * `<connex-segmented-control>`. Use to switch between translations of the
 ...
 *
 * @fires connex-language-change - Bubbles, composed; detail = `{ value }` on user selection.
 */
@customElement('connex-language-tabs')
export class ConnexLanguageTabs extends LitElement {
  static styles = css`
    :host {
      display: block;
    ...
    .divider {
      ...
      height: var(--connex-border-width-1);
      background: var(--connex-border-secondary);
    }
  `;
  ...
  private _resolveLanguages(): LanguageTabSpec[] {
    ...
    if (dropped.length > 0) {
      ...
      console.warn(
        `[connex-language-tabs] Unsupported language(s) ignored: ${dropped.join(', ')}. ` +
          `Connex language tabs only support English (en), Spanish (es), and French (fr).`,
      );
    }
  ...
  private _onSegmentedChange = (e: Event) => {
    ...
    this.dispatchEvent(
      new CustomEvent<{ value: SupportedLanguageValue }>('connex-language-change', {
        bubbles: true,
        composed: true,
  ...
  render() {
    ...
    return html`
      ...
      <connex-segmented-control
        ...
        @connex-segmented-change=${this._onSegmentedChange}
      >
        ${langs.map(
          (l) => html`
            <connex-segment value=${l.value} ?disabled=${l.disabled ?? false}>
              ${l.label}
            </connex-segment>
          `,
        )}
...
declare global {
  interface HTMLElementTagNameMap {
    'connex-language-tabs': ConnexLanguageTabs;
  }
}
